...
export const apiShowcaseGroups: ApiShowcaseGroup[] = [
	{
		description: "Public metadata and authenticated connection checks.",
		routes: [
			{ auth: "public", method: "GET", path: "/", purpose: "Public service metadata" },
			{ auth: "bearer", method: "GET", path: "/v1", purpose: "Authenticated version probe" },
		],
	...
	{
		description:
			"Unified list and detail routes shared by DJY, KZG, MHW, NBA, NTD, RFA, SOH, and ZJW.",
		routes: [
			{
				auth: "bearer",
				method: "GET",
		...
		routes: [
			{
				auth: "bearer",
				method: "POST",
				path: "/v1/media-resolve/:source",
				purpose: "Classify one media URL",
			},
			{
				auth: "bearer",
				method: "POST",
				path: "/v1/media-proxy/:source",
				purpose: "Proxy and stream media bytes",
			},
		],
